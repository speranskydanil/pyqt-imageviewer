...
class ImageViewer(QtGui.QWidget):
  def __init__(self, parent=None, image=None, scale=1.0, horizontal_position=0.5, vertical_position=0.5):
    ...
    self.t = TimeoutThread(0.1, self.configure_positions, self.horizontal_position, self.vertical_position)
  ...
  def configure_positions(self, horizontal_position=0.5, vertical_position=0.5):
    self.horizontal_position = horizontal_position
    self.vertical_position = vertical_position

    scroll_bar = self.scrollArea.horizontalScrollBar()
    scroll_bar.setValue(self.horizontal_position * self.imageLabel.width() - scroll_bar.pageStep() / 2)

    scroll_bar = self.scrollArea.verticalScrollBar()
    scroll_bar.setValue(self.vertical_position * self.imageLabel.height() - scroll_bar.pageStep() / 2)

  def zoom_out(self):
    if self.image:
      self.remember_positions()
      self.configure_scale(1.0 / 1.2 * self.scale)
      self.configure_positions(self.horizontal_position, self.vertical_position)
  ...
  def remember_positions(self):
    scroll_bar = self.scrollArea.horizontalScrollBar()
    self.horizontal_position = (scroll_bar.value() + scroll_bar.pageStep() / 2.0) / self.imageLabel.width()

    scroll_bar = self.scrollArea.verticalScrollBar()
    self.vertical_position = (scroll_bar.value() + scroll_bar.pageStep() / 2.0) / self.imageLabel.height()
